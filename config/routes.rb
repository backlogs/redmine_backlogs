ActionController::Routing::Routes.draw do |map|
  ...
  map.resource :rb, :only => :none do |rb|
    ...
    rb.resource   :wiki,             :only => [:show, :edit],      :controller => :rb_wikis,            :as => "wikis/:project_id/:sprint_id"
    ...
    rb.resource   :burndown_chart,   :only => :show,               :controller => :rb_burndown_charts,  :as => "burndown_charts/:project_id/:sprint_id"
    rb.resource   :impediment,       :except => :index,            :controller => :rb_impediments,      :as => "impediment/:id"
    rb.resources  :impediments,      :only => :index,              :controller => :rb_impediments,      :as => "impediments/:project_id/:sprint_id"
    ...
    rb.resource   :sprint,           :only => [:show, :update],    :controller => :rb_sprints,          :as => "sprints/:project_id/:sprint_id"
    rb.resource   :server_variables, :only => :show,               :controller => :rb_server_variables, :as => "server_variables/:project_id"
    rb.resource   :taskboard,        :only => :show,               :controller => :rb_taskboards,       :as => "taskboards/:project_id/:sprint_id"
