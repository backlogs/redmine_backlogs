  ...
  namespace :backlogs do
    ...
    task :fix_positions => :environment do
      if RbStory.trackers.size > 0
        ...
        newpos = 0
        RbStory.find_by_sql("SELECT COALESCE(MAX(position), -1) + 1 AS newpos FROM issues WHERE NOT position IS NULL AND tracker_id IN (#{RbStory.trackers(:type => :string)})").each do |row|
          newpos = row[0].to_i
        end
        ...
        RbStory.connection.execute("
          ...
          WHERE (issue_id IS NULL OR position <> new_position)
                AND tracker_id IN (#{RbStory.trackers(:type => :string)})").each do |row|
        ...
        RbStory.connection.execute("SELECT COUNT(*) AS not_updated FROM issues WHERE position IS NULL AND tracker_id IN (#{RbStory.trackers(:type => :string)})").each do |row|
