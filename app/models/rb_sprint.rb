...
class Burndown
  def initialize(sprint, burn_direction)
    ...
    @data[:points_required_burn_rate] = active.collect{|i| if days == i then Float(@data[:points_to_resolve][i]) else Float(@data[:points_to_resolve][i]) / ((days - i) + 0.001) end}
    @data[:hours_required_burn_rate] = active.collect{|i|  if days == i then Float(@data[:hours_remaining][i]) else Float(@data[:hours_remaining][i]) / ((days - i) + 0.001) end}
